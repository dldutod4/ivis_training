...
// 어떤 알고리즘의 경우 미리 계산된 테이블이 필요할 텐데,
// 이러한 정보들 역시 class 변수로 처리하셔도 됩니다. (난이도 : 上)

// https://m.blog.naver.com/fbfbf1/222202937028 에서 참고했지만
// find_kmp함수에서 pi배열을 생성하는 부분을 이해하지 못했음
...
public:
    ...
    int find_kmp(int find_from, const MyString &str) const;
...
MyString &MyString::erase(int loc, int num)
{
    ...
    if (num + loc > string_length)
        num = string_length - loc; //  범위를 문자열길이보다 길지 못하게 제한.
...
int MyString::find_kmp(int find_from, const MyString &str) const
{
    // pi배열 생성
    ...
    {
        for (int i = 0; i + begin < len; i++)
        {
            if (str.string_content[begin + i] != str.string_content[i])
                break;
            pi[begin + i] = std::max(pi[begin + i], i + 1);
        }
    }
    int begin = find_from, matched = 0;
    int n = string_length, m = len;
    while (begin <= n - m)
    {
        // 만약 H의 부분 문자열의 N과 같다면
        if (matched < m && string_content[begin + matched] == str.string_content[matched])
        {
            // 한 개가 일치하니 ++해준다
            ++matched;
...
int main()
{
    ...
    MyString str2("abcde");
    MyString str3("abaabab");
    str2.find_kmp(0, str3);
