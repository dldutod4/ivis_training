...
int MyString::find(int find_from, const MyString& str) const {
  ...
  //KMP pi finding "ababc"
  int m = str.string_length;
  char* ptr = &str.string_content[0];
  int* pi = new int[m];
  int begin = 1, matched = 0;
  while (begin + matched < m)
  {
    if(ptr[begin+matched]==ptr[matched])
    {
      matched++;
  ...
  //KMP Searchig
  while(i+m<=string_length)
  {
   if(j < m && string_content[i+j] == ptr[j])
   {
    j++;
    if(j==m)return i;//문자열이 모두 일치
   }
   else
   {
    if(j==0)i++;//일치하는 부분이 없음 따라서 시작 포인트를 한칸 오른쪽으로 이동
    else
    {
      i += j - pi[j-1];//pi 배열을 이용해 다음 시작지점으로 점프
      j = pi[j-1];//pi 배열을 이용해 이미 일치하는 부분을 건너뛰고 탐색 시작을 위해 대입
    }
   }
  }
...
int main() {
  MyString str1("aaaababaacdefababcbaa");
  MyString str2("ababc");

  std::cout << "str1 and str2 compare : " << str1.compare(str2) << std::endl;
  std::cout << str1.find(0, str2) << std::endl;
}